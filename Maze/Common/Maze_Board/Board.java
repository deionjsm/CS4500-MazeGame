...
/**
 * This class implements a simple Maze Board.Board for Maze.com.
 * CONSTANTS:
...
public class Board implements IBoardModel {
    ...
    //CTOR that enforces the every OTHER row/col can be shifted rule.
    public Board(int rows, int columns) {
        this.height = rows;
        this.length = columns;
        this.legalIndex = new ArrayList<>();
        for (int i = 0; i < this.height; i++) {
            //TODO ADD FLEXIBILITY TO NON SQUARE BOARDS
            if (i%2 == 0) { //indicies 1 3 5 on 7 by 7
                this.legalIndex.add(i);
            }
    ...
    //GIVES STANDARD RULESET OF ONLY shifting outermost and every other
    public Board(int rows, int cols, List<List<IMazePosition>> positions, Queue<ITile> std, boolean StandardRuleset) {

        this.legalIndex = new ArrayList<>();
        for (int i = 0; i < rows; i++) {
            //TODO ADD FLEXIBILITY TO NON SQUARE BOARDS
            if (i%2 == 0) { //indicies 0 2 4 6 on 7 by 7
                this.legalIndex.add(i);
            }
        ...
        this.height = rows;
        this.length = cols;
        this.mazeBoard = positions;
        this.spareTiles = std;
    }

    public List<List<IMazePosition>> getPositions() {
    ...
    public boolean shiftRowCol(int index, BoardOpt rORc, Direction direction) {
        ...
        if (!((rORc == BoardOpt.Row && (direction == Direction.Right || direction == Direction.Left))
            ...
            throw new IllegalArgumentException("Row and Enums.Direction missmatch");
        }
    ...
    private void shiftTiles(int index, Direction direction) {
        ...
        switch (direction) {
            ...
            case Down:
                ...
                break;
            default:
                throw new IllegalArgumentException("Unexpected Enums.Direction");
    ...
    public void undoMove(IMove move) {
        ...
        int movesRot = move.getRotation();
        //if past rotation was 0, should not rotate old placed tile at all from how it was previously -> what it is now
        //if 90, must rotate 270 degrees from what it is now
        //180 needs 180
        //270 needs 90
    ...
    @Override
    public IBoardModel Filtered() {

        List<List<IMazePosition>> posns = new ArrayList<>();
        ArrayDeque<ITile> spares = new ArrayDeque<>();
        spares.add(getSpareCopy());
        for (int row = 0; row < this.height; row++) {
            ArrayList<IMazePosition> temprow = new ArrayList<>();
            for (int col =0; col < this.length; col++) {
                temprow.add(this.mazeBoard.get(row).get(col).getCopy());
            }
            posns.add(temprow);
        }
        if (legalIndex.size() > 0) {
            return new Board(this.height, this.length, posns, spares, true);
        }
        return new Board(this.height, this.length, posns, spares);
    }

    @Override
    public ITile getSpareCopy() {
        ITile temp = new MazeTileSimple(this.spareTiles.peek().getConnector());
        temp.rotateConnector(this.spareTiles.peek().getRotationCW());
        return temp;
    }

    @Override
    public String getConnectorAt(int row, int col) {
        return this.mazeBoard.get(row).get(col).getTile().getString();
    }
    ...
    public List< List<IMazePosition> > getState() {
        return mazeBoard;
    }
    public ITile getSpare() {
